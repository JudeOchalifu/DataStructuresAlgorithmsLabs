...
public class Creature {
    ...
    private int dir; 
    private int step;
    //Create a creature of the given species with the given position in the world 
	...
	this.dir = dir;
	step = 1;
	//have the creature update the world map with the new creature 
    ...
    //Read in the opCode and address and call the creature's matched method 
    ...
    //hop one step in the direction that the creature is facing in 
    protected void hop(){
    ...
    //infect a given creature and set its address to a certain value 
	...
	if(victim!=null && victim.species()!=this.species() ){
	    ...
	    victim.doInstruction(10,address); 
	    WorldMap.displaySquare( victim.position(), this.species().getName().charAt(0), victim.direction(), this.species().getColor());
	}
    }
    //conditional checks, note that if the condition isn't true, these methods automatically update step 
    ...
    /**
       Test code to make sure that creature works 
     */
